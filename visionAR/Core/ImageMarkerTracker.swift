...
final class ImageMarkerTracker {
    ...
    private func makeProvider() throws -> ImageTrackingProvider {
        var referenceImages: [ImageTrackingProvider.ReferenceImage] = []
        for marker in referenceMarkers {
            let cgImage = try loadCGImage(for: marker)
            var referenceImage = try ImageTrackingProvider.ReferenceImage(image: cgImage,
                                                                          physicalWidth: marker.physicalWidth)
    ...
    private func handle(update: ImageTrackingProvider.AnchorUpdate) {
        switch update {
